    ...
    try:
        ...
        if "nipd" in eRVC_df.columns and "nomCeller" in eRVC_df.columns:
            ...
            if seleccion:
                ...
                # Análisis temporal por fecha de pesada
                st.markdown("### 📅 Análisis Temporal por Fecha")
                
                # Verificar si existe la columna dataPesada en ambos datasets
                if 'dataPesada' in extranet_df.columns and 'dataPesada' in ervc_df.columns:
                    # Filtrar datos por nipd seleccionados
                    extranet_temporal = extranet_df[extranet_df['nipd'].isin(selected_nipd)].copy()
                    ervc_temporal = ervc_df[ervc_df['nipd'].isin(selected_nipd)].copy()
                    
                    if not extranet_temporal.empty and not ervc_temporal.empty:
                             # Convertir dataPesada a datetime si no lo está
                             extranet_temporal['dataPesada'] = pd.to_datetime(extranet_temporal['dataPesada'], errors='coerce')
                             ervc_temporal['dataPesada'] = pd.to_datetime(ervc_temporal['dataPesada'], errors='coerce')
                             
                             # Agrupar por fecha y sumar kg
                             extranet_daily = extranet_temporal.groupby('dataPesada')['kg'].sum().reset_index()
                             extranet_daily.columns = ['Fecha', 'Kg_Extranet']
                             
                             ervc_daily = ervc_temporal.groupby('dataPesada')['kgTotals'].sum().reset_index()
                             ervc_daily.columns = ['Fecha', 'Kg_eRVC']
                             
                             # Combinar ambos datasets
                             daily_comparison = pd.merge(extranet_daily, ervc_daily, on='Fecha', how='outer').fillna(0)
                             daily_comparison = daily_comparison.sort_values('Fecha')
                             ...
                             if not daily_comparison.empty:
                                 ...
                                 fig.add_trace(go.Scatter(
                                     x=daily_comparison['Fecha'],
                                     y=daily_comparison['Kg_Extranet'],
                                     mode='lines+markers',
                                     name='Kg Extranet',
                                 ...
                                 fig.update_layout(
                                     title='📈 Evolución Temporal de Kg por Fecha de Pesada',
                                     xaxis_title='Fecha de Pesada',
                                 ...
                                 st.markdown("### 📊 Tabla de Totales Diarios")
                                 
                                 # Calcular diferencias
                                 daily_comparison['Diferencia_Kg'] = daily_comparison['Kg_eRVC'] - daily_comparison['Kg_Extranet']
                                 daily_comparison['Diferencia_Porcentual'] = (
                                     (daily_comparison['Diferencia_Kg'] / daily_comparison['Kg_eRVC']) * 100
                                 ).round(2)
                                 daily_comparison['Diferencia_Porcentual'] = daily_comparison['Diferencia_Porcentual'].fillna(0)
                                 
                                 # Formatear la tabla
                                 display_table = daily_comparison.copy()
                                 display_table['Fecha'] = display_table['Fecha'].dt.strftime('%Y-%m-%d')
                                 display_table['Kg_Extranet'] = display_table['Kg_Extranet'].round(2)
                                 display_table['Kg_eRVC'] = display_table['Kg_eRVC'].round(2)
                                 display_table['Diferencia_Kg'] = display_table['Diferencia_Kg'].round(2)
                                 ...
                                 st.dataframe(
                                     ...
                                     column_config={
                                         ...
                                         "Kg_Extranet": st.column_config.NumberColumn("🏭 Kg Extranet", format="%.2f"),
                                         "Kg_eRVC": st.column_config.NumberColumn("📋 Kg eRVC", format="%.2f"),
                                         "Diferencia_Kg": st.column_config.NumberColumn("⚖️ Diferencia Kg", format="%.2f"),
                                         "Diferencia_Porcentual": st.column_config.NumberColumn("📊 Diferencia %", format="%.2f%%")
                                     }
                                 )
                                 ...
                                 with col1:
                                     if ui is not None:
                                         ui.metric_card(
                                             ...
                                             content=f"{daily_comparison['Kg_Extranet'].mean():.0f} kg",
                                             description="Extranet",
                                             key="avg_extranet"
                                         )
                                     else:
                                         st.metric("Promedio Diario Extranet", f"{daily_comparison['Kg_Extranet'].mean():.0f} kg")
                                 
                                 with col2:
                                     if ui is not None:
                                         ui.metric_card(
                                             title="Promedio Diario",
                                             content=f"{daily_comparison['Kg_eRVC'].mean():.0f} kg",
                                             description="eRVC",
                                             key="avg_ervc"
                                         )
                                     else:
                                         st.metric("Promedio Diario eRVC", f"{daily_comparison['Kg_eRVC'].mean():.0f} kg")
                                 
                                 with col3:
                                     if ui is not None:
                                         ui.metric_card(
                                             title="Diferencia Promedio",
                                             content=f"{daily_comparison['Diferencia_Kg'].mean():.0f} kg",
                                             description="Diaria",
                                             key="avg_diff"
                                         )
                                     else:
                                         st.metric("Diferencia Promedio", f"{daily_comparison['Diferencia_Kg'].mean():.0f} kg")
                                 
                                 with col4:
                                     if ui is not None:
                                         ui.metric_card(
                                             title="Días Analizados",
                                             content=str(len(daily_comparison)),
                                             description="Total",
                                             key="total_days"
                                         )
                                     else:
                                         st.metric("Días Analizados", len(daily_comparison))
                             else:
                                 st.warning("⚠️ No hay datos temporales para mostrar en el rango seleccionado")
                         else:
                             st.warning("⚠️ No hay datos disponibles para las bodegas seleccionadas")
                     else:
                         st.error("❌ No se encontró la columna 'dataPesada' en los datasets")
        else:
